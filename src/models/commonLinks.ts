...
export interface CommonLinksData {
    id: string;
    groupId: string;
    link: string;
    label: string;
...
export const readCommonLinks = async (groupId: string) => {
    try {
        const checkWorkRef = collection(db, "common_links");
        const q = query(checkWorkRef, where("group_id", "==", groupId));
        ...
        return {
            success: false, error: "Failed to read common links"
        };
    }
}